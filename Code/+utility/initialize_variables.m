function f = initialize_variables(N, M, V, min_range, max_range, problem_number)

    %% function f = initialize_variables(N, M, V, min_tange, max_range, problem_number) 
    ...
    % Preallocate population matrix (decision vars + objectives)
    f = zeros(N, K);  
    ...
    for i = 1 : N
        ...
        f(i,V + 1: K) = utility.evaluate_objective(f(i,:), M, V, problem_number);
    end
end