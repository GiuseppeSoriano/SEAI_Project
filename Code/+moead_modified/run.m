function A = run(pop, gen, problem_number, scalarization_type)
    ...
    T = 10;                 % Neighborhood size (must be ≤ pop)
    ...
    % - mu: distribution index for Simulated Binary Crossover (SBX). Higher values generate offspring closer to parents.
    ...
    for i = 1:gen
        for subproblem = 1:pop
            ...
            % Two parents are randomly selected from the T closest subproblems (neighbors) of the current one.
            neighbor_indices = B(subproblem, randperm(T, 2));
            parent_1 = chromosome(neighbor_indices(1), :);
            parent_2 = chromosome(neighbor_indices(2), :);
            ...
            offspring = moead_modified.genetic_operator([parent_1; parent_2], M, V, mu, mum, min_range, max_range, problem_number);
            ...
            for neighbor = 1:T
                ...
                fitness_neighbor = moead_modified.scalarization( ...
                ...
                fitness_offspring = moead_modified.scalarization( ...
    ...
    % fronts = moead_modified.non_domination_sort_points(f_objs);
